...
<div class="w-full mx-auto mt-8 max-w-4xl flex flex-col h-[calc(100vh-8rem)] overflow-hidden">
    <h1 class="text-4xl font-extrabold mb-6 text-center text-gray-100">Leaderboard</h1>
    <div class="bg-gray-800 px-2 py-6 lg:p-6 lg:rounded-lg shadow-xl flex-1 overflow-hidden">
        {#if !$rankers.fetching || (sortedRankers?.length ?? 0) > 0}
            <div class="flex flex-col h-full overflow-visible">
                ...
                <div class="flex-1 overflow-x-hidden overflow-y-auto space-y-4 pt-4">
